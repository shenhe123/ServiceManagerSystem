...
public class WheelTime {
    ...
    private void setLunar(int year, final int month, int day, boolean isLeap, int h, int m, int s) {
        ...
        if (gravity.length != 6) {
            throw new RuntimeException("gravity[] length is not 6");
        }

        wv_year.setGravity(gravity[0]);
        wv_month.setGravity(gravity[1]);
        wv_day.setGravity(gravity[2]);
        wv_hours.setGravity(gravity[3]);
        wv_minutes.setGravity(gravity[4]);
        wv_seconds.setGravity(gravity[5]);
        // 添加"年"监听
